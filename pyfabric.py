...
def send_nc(xml_string, switch, username, password, operation):
	...
	with manager.connect(host=switch, port=830, username=username,password=password,hostkey_verify=False) as m:
		if operation == "edit":
			assert(":validate" in m.server_capabilities)
			m.edit_config(target='running', config=snippet, test_option='test-then-set',error_option=None)
			output = None	
		elif operation == "get":
			assert(":validate" in m.server_capabilities)
			output = m.get(('subtree', snippet))
